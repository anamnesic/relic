...
DistributedSpeculativeEngine::DistributedSpeculativeEngine(
    ...
    std::shared_ptr<Tokenizer> tokenizer) : target_decoder_(std::move(target_decoder)),
...
SpeculativeResult DistributedSpeculativeEngine::step_speculation(const std::vector<int> &context_tokens, int max_draft)
{
    ...
    if (context_tokens.empty() || !target_decoder_ || !target_model_)
        return res;
    ...
    for (int d = 0; d < max_draft; d++)
    {
        if (draft_decoder_ && draft_model_)
        {
            ...
            if (tokenizer_ && cur_token == tokenizer_->eos_id)
                break;
        }
        else
        {
    ...
    // 2. Real Batched Verification Phase on Target Device (GTX 1650)
    auto t_verify_start = std::chrono::high_resolution_clock::now();
    if (!draft_tokens.empty())
    {
        int64_t n_vocab = target_model_->n_vocab;
        std::vector<float> batched_logits((size_t)draft_tokens.size() * (size_t)n_vocab, 0.0f);

        // Single batched forward pass on target
        int batch_res = target_decoder_->forward_batch(*target_model_, draft_tokens, (int64_t)(context_tokens.size() - 1), batched_logits.data());
        if (batch_res == 0)
        {
            for (size_t i = 0; i < draft_tokens.size(); i++)
            {
                const float *logits_slice = batched_logits.data() + i * (size_t)n_vocab;
                int target_pred = (int)(std::max_element(logits_slice, logits_slice + n_vocab) - logits_slice);
                ...
                if (target_pred == draft_tok)
                {
                    ...
                    res.num_accepted++;
                    if (tokenizer_ && draft_tok == tokenizer_->eos_id)
                        break;
                }
                else
                {
                    // Rejection: append target's correction token and terminate draft sequence
